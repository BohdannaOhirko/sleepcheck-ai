    ...
    >
      ...
      <div className="absolute top-32 right-[15%] hidden lg:block pointer-events-none">
        <div className="relative animate-float">
          <div className="bg-white rounded-2xl shadow-lg border border-gray-100 px-4 py-3 flex items-center gap-3">
            <div className="w-8 h-8 rounded-xl flex items-center justify-center text-sm" style={{ background: 'linear-gradient(135deg, #d0f0da, #a8e6bc)' }}>
              😴
            </div>
            <div>
              <p className="text-xs font-semibold text-gray-800">Аналіз завершено</p>
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 py-24 w-full">
        <div className="max-w-3xl mx-auto text-center">
          ...
          {/* Бейдж */}
          <div className={`inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white border border-green-100 shadow-sm mb-8 transition-all duration-700 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
            <span className="relative flex h-2 w-2">
              <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-green-400 opacity-75" />
              <span className="relative inline-flex rounded-full h-2 w-2 bg-green-500" />
            </span>
            <span className="text-sm font-medium text-gray-600">Медичний центр &quot;Ехокор&quot;</span>
          </div>

          {/* Заголовок */}
          <h1 className={`text-5xl sm:text-6xl lg:text-7xl font-bold leading-[1.1] mb-6 transition-all duration-700 delay-100 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
            <span className="bg-clip-text text-transparent"
              style={{ backgroundImage: 'linear-gradient(135deg, var(--logo-default), var(--logo-green))' }}>
              AI‑діагностика
            </span>
            <br />
            ...
            <span className="relative text-gray-900">
              ...
              <svg className="absolute -bottom-2 left-0 w-full" height="8" viewBox="0 0 300 8" fill="none">
                <path d="M0 6 Q75 1 150 6 Q225 11 300 6" stroke="url(#ul)" strokeWidth="2.5" strokeLinecap="round" fill="none"/>
                <defs>
          ...
          {/* Опис */}
          <p className={`text-lg sm:text-xl text-gray-500 mb-10 max-w-2xl mx-auto leading-relaxed transition-all duration-700 delay-200 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
            ...
            <span className="font-semibold" style={{ color: 'var(--logo-green)' }}>AI‑асистента сомнолога</span>.
          </p>

          {/* CTA кнопки */}
          <div className={`flex flex-col items-stretch sm:flex-row sm:items-center sm:justify-center gap-3 mb-14 px-4 sm:px-0 transition-all duration-700 delay-300 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
  <Link href="/questionnaire" className="w-full sm:w-auto">
    <button className="group relative w-full sm:w-auto px-8 py-4 rounded-2xl font-semibold text-white text-base overflow-hidden shadow-lg hover:shadow-xl hover:shadow-green-200 transition-all duration-300 hover:-translate-y-0.5"
  ...
  >
    <span className="flex items-center justify-center gap-2">
      ...
      Консультація AI‑асистента
          ...
          {/* Картки переваг */}
          <div className={`grid grid-cols-1 sm:grid-cols-3 gap-4 max-w-2xl mx-auto transition-all duration-700 delay-400 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
            {[
              { icon: '✓', color: 'var(--logo-green)', bg: '#e8f7ed', title: 'Безкоштовно', sub: 'Онлайн анкета та базовий аналіз' },
              { icon: '◷', color: 'var(--logo-aqua)', bg: '#e8f4fd', title: '5 хвилин', sub: 'Швидкий AI‑аналіз та результат' },
              { icon: '⊘', color: 'var(--logo-default)', bg: '#edf3fd', title: 'Без реєстрації', sub: 'Анонімно та конфіденційно' },
              ...
              <div key={title}
                ...
                <div className="w-10 h-10 rounded-xl flex items-center justify-center mx-auto mb-3 text-lg font-bold transition-transform group-hover:scale-110"
          ...
          {/* Соціальний доказ */}
          <div className={`mt-10 flex flex-wrap items-center justify-center gap-6 transition-all duration-700 delay-500 ${isVisible ? 'opacity-100' : 'opacity-0'}`}>
            <div className="flex items-center gap-2 text-sm text-gray-400">
              ...
              <span><strong className="text-gray-700">10,000+</strong> пацієнтів</span>
            </div>
            <div className="w-px h-4 bg-gray-200 hidden sm:block" />
            {['AI від сомнологів', 'Медичний стандарт', 'GDPR захист'].map((item) => (
              <div key={item} className="flex items-center gap-1.5 text-sm text-gray-400">
                <svg className="w-3.5 h-3.5" style={{ color: 'var(--logo-green)' }} fill="currentColor" viewBox="0 0 20 20">
