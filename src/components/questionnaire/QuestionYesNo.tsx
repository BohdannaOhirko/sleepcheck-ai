  ...
  return (
    <div className="grid grid-cols-2 gap-4">
      ...
      >
        <div className="flex flex-col items-center gap-4">
          <div className={`w-16 h-16 rounded-full flex items-center justify-center transition-all ${
            value === 'yes' ? 'bg-[var(--logo-green)] text-white' : 'bg-secondary text-muted-foreground'
          }`}>
            <svg className="w-8 h-8" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
            </svg>
          </div>
          <div className={`text-2xl font-bold ${value === 'yes' ? 'text-[var(--logo-green)]' : 'text-foreground'}`}>
            Так
          </div>
        </div>
      </button>
      ...
      >
        <div className="flex flex-col items-center gap-4">
          <div className={`w-16 h-16 rounded-full flex items-center justify-center transition-all ${
            value === 'no' ? 'bg-[var(--logo-aqua)] text-white' : 'bg-secondary text-muted-foreground'
          }`}>
            <svg className="w-8 h-8" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
            </svg>
          </div>
          <div className={`text-2xl font-bold ${value === 'no' ? 'text-[var(--logo-aqua)]' : 'text-foreground'}`}>
            Ні
          </div>
        </div>
      </button>
